...
var createElement = function createElement(tag) {
  ...
  if (classnames.length) {
    ...
    (_node$classList = node.classList).add.apply(_node$classList, _toConsumableArray(classnames));
  }
...
function uploadInit(selector) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var files = [];
  var fileInput = document.querySelector(selector);
  var openBtn = createElement('button', ['btn'], 'Открыть');
  var previewContainer = createElement('div', ['preview']);
  var upload = createElement('button', ['btn', 'primary'], 'Загрузить');
  ...
  var handleRemove = function handleRemove(event) {
    ...
    var block = previewContainer.querySelector("[data-name=\"".concat(name, "\"]")).closest('.preview-image');
...
(0, _upload.uploadInit)('#inputUpload', {
  multy: true
});
